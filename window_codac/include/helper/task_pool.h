...
constexpr size_t thread_pool_size{15};
...
void TaskPool<PARAM>::HandelThread(const size_t thread_id)
{
    ...
    {
        if (false == param_task_queue_.IsEmpty())
        {
            auto msg = param_task_queue_.Pop();
            if (msg.second && (msg.first.handler != nullptr))
                msg.first.handler(std::move(msg.first.data), thread_id);
        }
    }
}
